...
export class WatchlistController {
  ...
  @ApiResponse({
    status: 200,
    description: 'The watchlist has been successfully retrieved.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async getWatchlist(@Req() req: { user: { uid: string } }) {
  ...
  async addToWatchlist(
    ...
    const res = await this.watchlistService.addMovieToWatchlist(
      req.user.uid,
      addMovieDto,
    );
    return res;
  ...
  async removeFromWatchlist(
    @Req() req: { user: { uid: string } },
    @Body() removeMovieDto: RemoveMovieDto,
  ) {
    const res = await this.watchlistService.removeMovieFromWatchlist(
      req.user.uid,
      removeMovieDto.movieId,
    );
    return res;
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'The movie has been successfully updated in the watchlist.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async updateWatchlist(
    ...
    const res = await this.watchlistService.updateMovieInWatchlist(
      req.user.uid,
      updateMovieDto,
    );
    return res;
  ...
  @ApiResponse({
    status: 200,
    description: 'The movie has been successfully found.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async searchMovie(
    @Req() req: { user: { uid: string } },
