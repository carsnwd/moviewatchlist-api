...
describe('WatchlistController', () => {
  ...
  describe('getWatchlist', () => {
    it('should return the user watchlist', async () => {
      ...
      const result = await controller.getWatchlist(mockExecutionContext.switchToHttp().getRequest());
  ...
  describe('addToWatchlist', () => {
    it('should add a movie to the watchlist', async () => {
      ...
      const result = await controller.addToWatchlist(mockExecutionContext.switchToHttp().getRequest(), mockAddMovieDto);

      expect(result).toEqual(mockWatchlist);
      expect(service.addMovieToWatchlist).toHaveBeenCalledWith('mockUid', mockAddMovieDto);
    });
  });
  ...
  describe('removeFromWatchlist', () => {
    it('should remove a movie from the watchlist', async () => {
      ...
      const mockWatchlist = { userId: 'mockUid', movies: [] };
      mockWatchlistService.removeMovieFromWatchlist.mockResolvedValue(mockWatchlist);

      const result = await controller.removeFromWatchlist(mockExecutionContext.switchToHttp().getRequest(), mockRemoveMovieDto);

      expect(result).toEqual(mockWatchlist);
      expect(service.removeMovieFromWatchlist).toHaveBeenCalledWith('mockUid', 'mockMovieId');
    });
  });

  describe('updateWatchlist', () => {
    it('should update a movie in the watchlist', async () => {
      const mockUpdateMovieDto: UpdateMovieDto = { movieId: 'mockMovieId', fileName: 'Updated Title' };
      const mockWatchlist = { userId: 'mockUid', movies: [mockUpdateMovieDto] };
      mockWatchlistService.updateMovieInWatchlist.mockResolvedValue(mockWatchlist);

      const result = await controller.updateWatchlist(mockExecutionContext.switchToHttp().getRequest(), mockUpdateMovieDto);
