...
describe('WatchlistService', () => {
    ...
    beforeEach(async () => {
        ...
        mockWatchlist = {
            userId: 'mockUserId',
            movies: [{ id: 'mockMovieId', title: 'Mock Movie', file_name: '', file_size: 0 }],
            save: jest.fn(),
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                WatchlistService,
                { provide: getModelToken(Watchlists.name), useValue: mockWatchlistModel },
    ...
    describe('removeMovieFromWatchlist', () => {
        it('should remove a movie from the watchlist', async () => {
            ...
            const result = await service.removeMovieFromWatchlist('mockUserId', 'mockMovieId');
        ...
        it('should handle movie not in watchlist', async () => {
            ...
            expect(result).toEqual({ userId: 'mockUserId', movies: [{ id: 'anotherMovieId', title: 'Another Movie' }] });
            expect(mockWatchlist.movies).toHaveLength(1);
            expect(mockWatchlist.save).toHaveBeenCalled();
        });

        it('should handle errors', async () => {
            mockWatchlistModel.findOne.mockRejectedValueOnce(new Error('Error removing movie from watchlist'));

            await expect(service.removeMovieFromWatchlist('mockUserId', 'mockMovieId')).rejects.toThrow('Error removing movie from watchlist');
        });
    });
    ...
    describe('getWatchlist', () => {
        it('should return watchlist if it exists', async () => {
            ...
            (watchlistModel.findOne as jest.Mock).mockResolvedValueOnce(mockWatchlist);
        ...
        it('should create and return a new watchlist if it does not exist', async () => {
            ...
            const watchlistInstance = { ...mockWatchlist, save: jest.fn().mockResolvedValue(true) };

            mockWatchlistModel.findOne.mockResolvedValueOnce(null);

            //@ts-ignore
            jest.spyOn(watchlistModel, 'create').mockImplementation(() => watchlistInstance);
        ...
        it('should throw an error if there is an exception', async () => {
            const userId = 'mockUserId';
            (watchlistModel.findOne as jest.Mock).mockRejectedValueOnce(new Error('Database error'));

            await expect(service.getWatchlist(userId)).rejects.toThrow('Error getting watchlist');
            expect(watchlistModel.findOne).toHaveBeenCalledWith({ userId });
        });
        ...
        it('should throw an error if when trying to create a watchlist there is an error', async () => {
            ...
            //@ts-ignore
            (watchlistModel.create as jest.Mock).mockRejectedValueOnce(new Error('Database error'));
            await expect(service.getWatchlist(userId)).rejects.toThrow('Error creating watchlist');
            expect(watchlistModel.create).toHaveBeenCalledWith({ userId });
        });
    });
    ...
    describe('addMovieToWatchlist', () => {
        it('when watchlist not found', async () => {
            ...
            //@ts-ignore
            await expect(service.addMovieToWatchlist(userId, movieDto)).rejects.toThrow('Error adding movie to watchlist');
        });
        it('should add a movie to the watchlist', async () => {
            const userId = 'mockUserId';
            const movieDto = { movieId: '1', fileName: 'Inception.mp4', fileSize: 2 };
            const mockMovie = { id: '1', title: 'Inception', file_name: '', file_size: '' };
        ...
        it('should throw an error if the movie is already in the watchlist', async () => {
            const userId = 'mockUserId';
            const movieDto = { movieId: 'mockMovieId', fileName: 'Mock Movie', fileSize: 2 };

            //@ts-ignore
            jest.spyOn(service, 'getWatchlist').mockResolvedValueOnce(mockWatchlist);

            (tmdbService.getMovieById as jest.Mock).mockReturnValueOnce(of({ id: 'mockMovieId', title: 'Mock Movie' }));

            await expect(service.addMovieToWatchlist(userId, movieDto)).rejects.toThrow('Movie already in watchlist');
        });
        ...
        it('should handle errors', async () => {
            ...
            //@ts-ignore
            jest.spyOn(service, 'getWatchlist').mockRejectedValueOnce(new Error('Error adding movie to watchlist'));
    ...
    describe('updateMovieInWatchlist', () => {
        it('should update a movie in the watchlist', async () => {
            const userId = 'mockUserId';
            const updateMovieDto = { movieId: 'mockMovieId', fileName: 'Updated Movie.mp4', fileSize: 2 };

            //@ts-ignore
            jest.spyOn(service, 'getWatchlist').mockResolvedValueOnce(mockWatchlist);

            const result = await service.updateMovieInWatchlist(userId, updateMovieDto);
        ...
        it('should throw an error if the movie is not found in the watchlist', async () => {
            const userId = 'mockUserId';
            const updateMovieDto = { movieId: 'nonExistentMovieId', fileName: 'Updated Movie.mp4', fileSize: 2 };

            //@ts-ignore
            jest.spyOn(service, 'getWatchlist').mockResolvedValueOnce(mockWatchlist);

            await expect(service.updateMovieInWatchlist(userId, updateMovieDto)).rejects.toThrow('Movie not found in watchlist');
        });
        ...
        it('should handle errors', async () => {
            ...
            jest.spyOn(service, 'getWatchlist').mockRejectedValueOnce(new Error('Error updating movie in watchlist'));

            await expect(service.updateMovieInWatchlist(userId, updateMovieDto)).rejects.toThrow('Error updating movie in watchlist');
        });
    });
    ...
    describe('searchMovie', () => {
        ...
        it('should handle errors', async () => {
            ...
            jest.spyOn(service, 'getWatchlist').mockRejectedValueOnce(new Error('Error searching movie in watchlist'));

            await expect(service.searchMovie(userId, query)).rejects.toThrow('Error searching movie in watchlist');
        });
    });
    ...
    describe('getMovie', () => {
        it('should return the movie if it exists in the watchlist', async () => {
            ...
            expect(result).toEqual({
                userId: mockWatchlist.userId,
                movie: { id: 'mockMovieId', title: 'Mock Movie', file_name: '', file_size: 0 },
            });
        });
        ...
        it('should handle errors', async () => {
            ...
            jest.spyOn(service, 'getWatchlist').mockRejectedValueOnce(new Error('Error getting movie from watchlist'));

            await expect(service.getMovie(userId, movieId)).rejects.toThrow('Error getting movie from watchlist');
        });
    });
});